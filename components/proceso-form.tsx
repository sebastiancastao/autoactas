...
const getApoderadoHelperText = (
  tipo: "acreedor" | "deudor",
  apoderadoNombre: string,
) => {
  const tipoLabel = tipo === "acreedor" ? "acreedor" : "deudor";
  const nombreLimpio = apoderadoNombre.trim();

  if (!nombreLimpio) {
    return `Sin apoderado asignado para este ${tipoLabel}.`;
  }

  return `Apoderado asignado para este ${tipoLabel}: ${nombreLimpio}.`;
};
          ...
          <div>
            ...
            {!acreedoresOpen && (
              <p className="text-[11px] text-zinc-500 dark:text-zinc-400">
                Despliega la sección para completar el resto de datos.
              </p>
            )}
          </div>
          <button
        ...
        <div>
          ...
          <p
            className={`mt-1 text-[11px] ${
              acreedor.apoderadoNombre.trim()
                ? "text-emerald-700 dark:text-emerald-300"
                : "text-zinc-500 dark:text-zinc-400"
            }`}
          >
            {getApoderadoHelperText("acreedor", acreedor.apoderadoNombre)}
          </p>
        </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white/80 shadow-sm dark:border-white/10 dark:bg-white/5">
                ...
                {deudoresForm[0] && (
                  <div className="border-t border-zinc-100 px-4 pb-3 pt-3 dark:border-white/5">
                    ...
                    <p
                      className={`mt-1 text-[11px] ${
                        deudoresForm[0].apoderadoNombre.trim()
                          ? "text-emerald-700 dark:text-emerald-300"
                          : "text-zinc-500 dark:text-zinc-400"
                      }`}
                    >
                      {getApoderadoHelperText("deudor", deudoresForm[0].apoderadoNombre)}
                    </p>
                  </div>
                )}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <p
                          className={`mt-1 text-[11px] ${
                            deudor.apoderadoNombre.trim()
                              ? "text-emerald-700 dark:text-emerald-300"
                              : "text-zinc-500 dark:text-zinc-400"
                          }`}
                        >
                          {getApoderadoHelperText("deudor", deudor.apoderadoNombre)}
                        </p>
                      </div>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-white/80 shadow-sm dark:border-white/10 dark:bg-white/5">
                <div className="flex items-center justify-between gap-3 p-4">
                  ...
                  <button
                    ...
                    className="h-9 shrink-0 rounded-full border border-zinc-200 bg-white px-3 text-xs font-semibold text-zinc-700 transition hover:border-zinc-900 hover:text-zinc-900 dark:border-white/10 dark:bg-white/5 dark:text-zinc-200 dark:hover:border-white dark:hover:text-white"
                  >
                    + Acreedor
                  </button>
                </div>

                <div className="space-y-4 border-t border-zinc-100 p-4 dark:border-white/5">
                  <div className="space-y-4">
                    {acreedoresForm.map(renderAcreedorRow)}
                  </div>

                  {acreedoresOpen && acreedoresForm.length > 0 && (
